...
from rails.demo import Resize_metrics, ToTensor_metrics, normalize_map, blend_map
...
class QCollector(AutonomousAgent):
    ...
    def setup(self, path_to_conf_file):
        ...
        self.device_id = int(np.random.randint(3, size=1)[0])
        ...
        # self.sage_model = Unet().to(self.device)
        # print("Model loaded! Loading Checkpoint...")
        # self.sage_model_name = models[1]
        # self.sage_state_dict = torch.load(os.path.join(model_dir, self.sage_model_name))
        # self.sage_model.load_state_dict(self.sage_state_dict)
        ...
        ego_model = EgoModel(1./FPS*(self.num_repeat+1)).to(self.device)
        ego_model.load_state_dict(torch.load(self.ego_model_dir))
        ego_model.eval()
        BellmanUpdater.setup(config, ego_model, device=self.device)
    ...
    def get_gaze(self, img):
        ...
        with torch.no_grad():
            sage_pred, _ = self.sage_model(img)
    ...
    def run_step(self, input_data, timestamp):
        ...
        delta_locs, delta_yaws, next_spds = BellmanUpdater.compute_table(yaw/180*math.pi, device=self.device)
